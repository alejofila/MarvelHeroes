...
import com.example.marvelchallenge.data.DataError.TooManyRequestsError
import com.example.marvelchallenge.data.DataError.UnauthorizedError
...
class DomainErrorMapper @Inject constructor() {
  ...
  fun map(error: DataError): DomainError {
    return when (error) {
      ...
      UnauthorizedError -> MissingAPIKey
      TooManyRequestsError -> RateExceeded
    }
  }
}
